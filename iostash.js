...
iostash = function () {
  ...
  init = function (token) {
    if (!token) {
      alert('accessToken not specified!')
      return
    }
    access_token = token
    ...
    socket = io(host, { query: 'accessToken=' + access_token,'reconnect': true, 'reconnection delay': 500,'max reconnection attempts': 10 })
    socket.on('connect', function () {})
  ...
  subscribeChannel = function (channelId, callback) {
    if (isPublic) {
      alert('Only public devices can be subscribed to in Public Mode')
      return
    }
  ...
  return {
    ...
    'subscribeDataPoint': subscribeDataPoint,
